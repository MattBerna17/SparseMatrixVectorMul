...
def run_rank_thread(executable, seq_executable, node, n, nz, mode, ranks, threads, repetitions_count, tolerance):
    rows = []
    cache = {}
    ...
    for rank in ranks:
        for thread in threads:
            for repetition, result in repetitions(
                ...
                reference, tolerance):
                add_time_row(rows, "rank_thread", executable, rank, thread, n, nz,
...
def main():
    ...
    for mode in ["regular", "irregular"]:
        print("running rank and thread impact")
        write_csv(args.output / f"{mode}_rank_thread.csv", run_rank_thread(
