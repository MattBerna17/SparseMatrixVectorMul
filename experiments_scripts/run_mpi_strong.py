...
def run_strong(executable, seq_executable, node, n, nz, mode, ranks_list, threads, repetitions_count, tolerance):
    rows = []
    cache = {}
    ...
    for ranks in ranks_list:
        for repetition, result in repetitions(
            ...
            reference, tolerance):
            add_time_row(rows, "strong", executable, ranks, threads, n, nz,
...
def main():
    ...
    for mode in ["regular", "irregular"]:
        print("running mpi strong scalability")
        write_csv(args.output / f"{mode}_strong.csv", run_strong(
