...
def run_chunks(executables, node, n, nz, mode, threads, chunk_sizes,
...
def main():
    ...
    executables = {"seq": args.seq, "openmp": args.openmp}
    args.output.mkdir(parents=True, exist_ok=True)

    for mode in ("regular", "irregular"):
        print(f"running openmp chunk sweep - {mode}")
        rows = run_chunks(
            executables, args.node, args.n, args.nz, mode,
            args.threads, chunk_sizes, args.repetitions, args.tolerance
        )
        write_csv(args.output / f"{mode}_openmp_chunks.csv", rows)
